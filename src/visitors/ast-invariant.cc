...
void ASTInvariant::Visit(const CharacterLiteralName &n) { }
...
void ASTInvariant::Visit(const IdentifierList &n) { assert(n.ids); }
void ASTInvariant::Visit(const IndexConstraint &n) { assert(n.indices); assert(n.indices->size()); }
void ASTInvariant::Visit(const IndexedName &n) { assert(n.prefix); assert(n.indices); }
...
void ASTInvariant::Visit(const ObjectDeclaration &n) { assert(n.names); assert(n.names->size()); }
...
void ASTInvariant::Visit(const RealConstraint &n) { assert(n.range); }
...
void ASTInvariant::Visit(const SelectedName &n) { assert(n.prefix); assert((n.selector == nullptr) && n.all); }
void ASTInvariant::Visit(const SimpleName &n) { }
void ASTInvariant::Visit(const SliceName &n) { assert(n.prefix); assert(n.range); }
